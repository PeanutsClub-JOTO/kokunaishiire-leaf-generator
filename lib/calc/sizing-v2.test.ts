...
describe('sizeSingleV2 — 単品（1商品パッケージ=1個, 卸価格÷入数で判定）', () => {
  it('塩レモン: 原価400 × 1ケース12 → 72個入, 卸41,400, 掲載単価575.0', () => {
    ...
    expect(r.wholesale).toBe(41400);
    expect(round1(r.unitPrice)).toBe(575.0);
    expect(r.minLotPrice).toBe(4800);
  });

  it('涼ごこち: 原価150 × 1甲60 → 180個入, 卸39,000', () => {
    const r = sizeSingleV2(150, 60);
    expect(r.leafQty).toBe(180);
    expect(r.costTotal).toBe(27000);
    expect(r.wholesale).toBe(39000);
    expect(round1(r.unitPrice)).toBe(216.7);
  });

  it('仕入原価合計と卸価格を分けて持つ（原価465×2ケース32 → 原価29,760, 卸42,680）', () => {
    const r = sizeSingleV2(465, 32);
    expect(r.costTotal).toBe(29760);
    expect(r.wholesale).toBe(42680);
  ...
  it('卸価格÷入数が1000円超なら企画対象外', () => {
    ...
    expect(r.wholesale).toBe(41400);
    expect(round1(r.unitPrice)).toBe(1725.0);
  });
...
describe('sizeAssortV2 — アソート', () => {
  ...
  it('条件が揃わないアソートは各商品の入数ロットを合算して判定する', () => {
    ...
    expect(r.wholesale).toBe(45560);
    expect(round1(r.unitPrice)).toBe(949.2);
  });
...
describe('canAssort — アソート可否の簡易判定', () => {
  ...
  it('0.75で割った時点で1000円を超える商品はアソート不可', () => expect(canAssort([460, 800])).toBe(false));
});
